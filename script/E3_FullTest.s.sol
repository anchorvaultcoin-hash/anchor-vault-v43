...
contract E3_FullTest is Script {
    function run() external {
        ...
        address newMain = 0x3333333333333333333333333333333333333333;
        address newRecovery = 0x4444444444444444444444444444444444444444;

        // 1. Отправитель пополняет получателя эфиром (на газ)
        vm.startBroadcast(pk);
        payable(recipient).transfer(0.01 ether);
        vm.stopBroadcast();

        // 2. Получатель устанавливает emergency (не равный себе)
        vm.startBroadcast(recKey);
        AnchorVaultV45(payable(vaultAddr)).setGlobalEmergency(emergencyAddr);
        vm.stopBroadcast();

        // 3. Отправитель создаёт токен, сейф, инициирует эскроу
        vm.startBroadcast(pk);
        ...
        // 4. Попытка подтвердить не-получателем (отправитель) — должен быть revert NotTransferRecipient
        vm.startBroadcast(pk);
