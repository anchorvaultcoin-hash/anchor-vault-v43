...
contract D2_EarlyCloseMain is Script {
    function run() external {
        uint256 pk = vm.envUint("PRIVATE_KEY");
        uint256 mainKey = vm.envUint("MAIN_KEY");    // main вместо recovery
        ...
        bytes32 TYPEHASH = keccak256("EarlyClose(address owner,uint256 vaultId,uint64 nonce,uint256 deadline)");
        ...
        AnchorVaultV45(payable(vaultAddr)).earlyClose(vid, deadline, sig);
        vm.stopBroadcast();
    }
}
