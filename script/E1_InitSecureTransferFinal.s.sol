...
contract E1_InitSecureTransferFinal is Script {
    function run() external {
        ...
        address recipient = 0x8d22bBDA101751805CB2dfa9b55AB79c353fAa47;
        address newMain = 0x1111111111111111111111111111111111111111;
        address newRecovery = 0x2222222222222222222222222222222222222222;

        // 1. Сначала пополним recipient (mainAuthKey) эфиром с основного кошелька
        vm.startBroadcast(pk);
        payable(recipient).transfer(0.01 ether);
        vm.stopBroadcast();

        // 2. Получатель устанавливает emergency
        vm.startBroadcast(mainKey);
        AnchorVaultV45(payable(vaultAddr)).setGlobalEmergency(0x2bd8946f52C6255710fC61a44f16875f8A56B4aC);
        vm.stopBroadcast();

        // 3. Отправитель создаёт сейф
        vm.startBroadcast(pk);
